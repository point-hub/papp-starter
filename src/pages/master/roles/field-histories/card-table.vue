...
import { getFieldHistoriesApi, type IGetFieldHistoriesData } from '@/composables/api/administrator/audit-logs/get-field-histories.api';
...
const {
  ...
  columns: {
    created_at: { label: 'Updated at', isVisible: true, isSelectable: false },
    operation_id: { label: 'Operation ID', isVisible: true, isSelectable: false },
    actor: { label: 'Actor', isVisible: true, isSelectable: false },
    before: { label: 'Before', isVisible: true, isSelectable: false },
    after: { label: 'After', isVisible: true, isSelectable: false },
  },
});
 ...
 */
const auditLogs = ref<IGetFieldHistoriesData[]>();
...
<template>
  <base-card :title="`Audits - ${route.params.field}`">
      ...
      <base-table>
        ...
        <tbody>
          ...
          <template v-if="!isLoading && auditLogs && auditLogs.length > 0">
            <tr v-for="(role, index) in auditLogs" :key="index">
