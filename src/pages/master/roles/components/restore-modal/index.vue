...
import { restoreRoleApi } from '@/composables/api/master/roles/restore.api';
...
const onRestore = async (reason: string) => {
  ...
  try {
    await restoreRoleApi(_id.value as string, {
      update_reason: reason,
    });
    toast(`Restore Role "${label.value}" success`, { color: 'success' });
    emit('restored');
  } catch (error) {
