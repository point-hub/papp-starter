...
import { archiveRoleApi } from '@/composables/api/master/roles/archive.api';
...
const onArchive = async (reason: string) => {
  ...
  try {
    await archiveRoleApi(_id.value as string, {
      update_reason: reason,
    });
    toast(`Archive Role "${label.value}" success`, { color: 'success' });
    emit('archived');
  } catch (error) {
